...
export const VoiceSelector = ({
  ...
  const currentOpen = openProp !== undefined ? openProp : open;

  const handleOpenChange = useCallback(
    (nextOpen: boolean, eventDetails?: unknown) => {
      setOpen(nextOpen);
      if (typeof onOpenChange === "function") {
        (onOpenChange as (open: boolean, details?: unknown) => void)(nextOpen, eventDetails);
      }
    },
    [onOpenChange]
  );
  ...
  const voiceSelectorContext = useMemo(
    ...
    [value, setValue, currentOpen, handleOpenChange]
  );

  return (
    <VoiceSelectorContext.Provider value={voiceSelectorContext}>
      <Dialog onOpenChange={handleOpenChange} open={currentOpen} {...props}>
        {children}
      </Dialog>
